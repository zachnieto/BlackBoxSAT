...
public class Hit extends AHint {
    ...
    /**
     * Constructs a Hit using the board dimensions, position of the ray, and
     * direction of travel.
     *
    ...
    // Generate the boolean expression that determines whether or not this Hit hint
    // is satisfied given some set of atoms (entering from the border--Section 3.5)
    public String generate() {
	// Corner case: if the CCW atom is out of bounds, then there must be no CW atom
	// to avoid border reflection
	...
	// Corner case: if the CW atom is out of bounds, then there must be no CCW atom
	// to avoid border reflection
	...
	// Normal entry case: there must be neither a CW atom nor a CCW atom to avoid
	// border reflection
	else {
	...
	if (this.outOfBounds(atomCW) && this.outOfBounds(atomCCW))
	    // then the only way for the hit to be satisfied is for there to be an atom at
	...
	// If the ray would exit on a counterclockwise deflection,
	    ...
	    // then the hit could be satisfied with an atom here, or with no
	    // counterclockwise deflection and an atom later.
	    return String.format("(P%d%d | (~P%d%d & ",
		    this.dir.getNextPosn(this.position).getX(),
		    this.dir.getNextPosn(this.position).getY(),
		    atomCCW.getX(),
	...
	// If the ray would exit on a clockwise deflection,
	    ...
	    // then the hit could be satisfied with an atom here, or with no clockwise
	    // deflection and an atom later.
	    return String.format("(P%d%d | (~P%d%d & ", 
		    this.dir.getNextPosn(this.position).getX(),
		    this.dir.getNextPosn(this.position).getY(),
		    atomCW.getX(),
	...
	// If the ray is somewhere in the middle of the board, then check for an atom
	// here or an atom after the ray continues in the deflected direction.
	else {
	    // Case 1: no deflection
	    return String.format("(P%d%d | ((~P%d%d & ~P%d%d) => ",
		    this.dir.getNextPosn(this.position).getX(),
		    this.dir.getNextPosn(this.position).getY(),
		    atomCW.getX(), atomCW.getY(), atomCCW.getX(),
		    atomCCW.getY())
		    + new Hit(
			    this.boardDim, 
			    dir.getNextPosn(this.position), 
			    this.dir)
