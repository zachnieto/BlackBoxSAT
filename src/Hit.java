...
public class Hit extends AHint {
	...
	@Override
	public String generate() { //problem: cycles? - must keep track of places we already visited
		...
		if( this.outOfBounds(upDeflection) && this.outOfBounds(downDeflection)   )
			...
			return String.format("(P%d%d | ((P%d%d => 0) & ~P%d%d => ",
						...
						downDeflection[0], downDeflection[1])
					+ new Hit(this.boardDim, this.getNextPosn()[0], this.getNextPosn()[1], dir).generate() + "))"; 
		}
			...
			Hit deflFromBelow = new Hit(this.boardDim, this.x, this.y, (this.dir - 1) % 4); 
			
			return String.format("(P%d%d | ((~P%d%d & ~P%d%d) =>  ", 
					this.x, this.y, 
					upDeflection[0], upDeflection[1],
					downDeflection[0], downDeflection[1])
				+ new Hit(this.boardDim, this.getNextPosn()[0], this.getNextPosn()[1], this.dir).generate()
				+ String.format(" & (P%d%d & ~P%d%d) => ",
						upDeflection[0], upDeflection[1], 
						downDeflection[0], downDeflection[1])
				+ new Hit(this.boardDim, deflFromAbove.getNextPosn()[0], deflFromAbove.getNextPosn()[1], (this.dir + 1) % 4).generate()
				+ String.format(" & (~P%d%d & P%d%d) => ",
						upDeflection[0], upDeflection[1], 
						downDeflection[0], downDeflection[1])
				+ new Hit(this.boardDim, deflFromBelow.getNextPosn()[0], deflFromBelow.getNextPosn()[1], (this.dir - 1) % 4).generate()
